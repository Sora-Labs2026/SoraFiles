...
async function initializeRuntime(): Promise<Runtime> {
  ...
  boundedReady.catch(() => {});
  const helper = new module.ZetaHelperMain(new URL(officeThreadUrl, document.baseURI).href, {
    threadJsType: 'module',
    wasmPkg: 'free',
  ...
  return { helper, ready: boundedReady, pending };
}

async function getRuntime(onProgress?: ProgressCallback, signal?: AbortSignal) {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    runtime.ready.then(resolve, reject).finally(() => signal?.removeEventListener('abort', abort));
  });
  return runtime;
}
...
export async function convertOfficeToPdf(
  ...
  const runtime = await getRuntime(options.onProgress, options.signal);
  if (options.signal?.aborted) throw new DOMException('Cancelled', 'AbortError');
