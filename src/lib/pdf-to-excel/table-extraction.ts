...
const typedValue = (raw: string): unknown => {
  const value = raw.replace(/\s+/g, ' ').trim();
  if (!value) return '';
  const numeric = value.replace(/[,$£€¥₹\s]/g, '').replace(/^\((.+)\)$/, '-$1');
  if (/^[+-]?(?:\d+\.?\d*|\.\d+)%$/.test(numeric)) return Number(numeric.slice(0, -1)) / 100;
...
const normalize = (items: PositionedTextItem[]): CellToken[] => items.flatMap((item) => {
...
const clusterRows = (tokens: CellToken[]): TextRow[] => {
...
const splitRegions = (rows: TextRow[]): TextRow[][] => {
  ...
  rows.forEach((row, index) => {
    if (index > 0) {
      ...
      if (gap > Math.max(normalGap * 2.4, rows[index - 1].height * 2.8)) regions.push([]);
    }
...
const columnAnchors = (rows: TextRow[]): number[] => {
...
const regionToTable = (rows: TextRow[], preserveText = false): ExtractedTable | null => {
  ...
  if (rows.length < 2 || populatedColumns < 2 || denseRows < Math.min(2, rows.length)) return null;
  return { rows: matrix, sourceRowCount: rows.length, columnCount: populatedColumns };
};

export function extractTablesFromTextItems(items: PositionedTextItem[], { preserveText = false } = {}): ExtractedTable[] {
  const tokens = normalize(items);
  if (!tokens.length) return [];
  const rows = clusterRows(tokens);
  const regions = splitRegions(rows);
  const tables = regions.map((region) => regionToTable(region, preserveText)).filter((table): table is ExtractedTable => Boolean(table));
