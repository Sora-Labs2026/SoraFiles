...
try{
 {const {context,page}=await create(1280,'light');await page.goto(base);await page.waitForTimeout(3800);assert.equal(await page.locator('[data-desktop-promo]').isVisible(),false);assert.equal((await page.request.get(base+'/desktop/redeem')).status(),404);checks.push('Production promotion remains disabled and no public Redeem page exists');await context.close();}
