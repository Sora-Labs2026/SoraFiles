...
test('Office container inspection distinguishes DOCX/XLSX/PPTX and rejects ambiguous or traversal ZIP entries',async()=>{const dir=await mkdtemp(join(tmpdir(),'sorafiles-office-type-'));try{
 const entry=strToU8('<synthetic/>'),base={'[Content_Types].xml':entry};
 for(const [name,extra,expected] of [['word',{'word/document.xml':entry},'DOCX'],['sheet',{'xl/workbook.xml':entry},'XLSX'],['slides',{'ppt/presentation.xml':entry},'PPTX'],['ambiguous',{'word/document.xml':entry,'xl/workbook.xml':entry},null],['mixed-slides',{'ppt/presentation.xml':entry,'xl/workbook.xml':entry},null],['traversal',{'../bad':entry,'word/document.xml':entry},null]]){const file=join(dir,name+'.zip');await writeFile(file,zipSync({...base,...extra}));assert.equal((await classifySelection([file]))[0].format,expected);}
 }finally{await rm(dir,{recursive:true,force:true});}});
