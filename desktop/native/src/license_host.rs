...
pub(crate) fn run_component(directory:&Path,runtime:&Path,entry:&Path,config:Value,action:&str,params:Value,store:&impl KeyStore)->Result<Value,String>{
 ...
 #[cfg(unix)] let mut child=crate::unix_process_group::ProcessGroup::spawn(&mut command).map_err(|_|"Desktop license component could not start")?;
 #[cfg(not(unix))] let mut child=command.spawn().map_err(|_|"Desktop license component could not start")?;
 ...
 #[cfg(unix)] let (receiver,reader)=crate::unix_process_group::FrameReader::start(output,MAX_FRAME as usize);
 #[cfg(not(unix))] let reader=std::thread::spawn(move||{let mut reader=BufReader::new(output);loop{let mut line=Vec::new();let read=reader.by_ref().take(MAX_FRAME+1).read_until(b'\n',&mut line);if !matches!(read,Ok(n) if n>0&&n<=MAX_FRAME as usize){break;}if sender.send(line).is_err(){break;}}});
