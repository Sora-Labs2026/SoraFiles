// Generated from src/lib/image/manual-adjustments.ts by sync-image-adjustments.mjs.
...
export const defaultManualAdjustments = () => ({
    exposure: 0,
    highlights: 0,
    shadows: 0,
    contrast: 0,
...
const clampControl = (value, minimum = -100) => Math.max(minimum, Math.min(100, Number(value) || 0));
const luma = (red, green, blue) => (54 * red + 183 * green + 19 * blue) >> 8;
export function normalizeManualAdjustments(value) {
    return {
...
export function hasManualAdjustments(value) {
    return manualAdjustmentKeys.some((key) => value[key] !== 0);
}
function reduceNoise(data, width, height, amount) {
    ...
    for (let y = 1; y < height - 1; y += 1) {
        ...
        for (let x = 1; x < width - 1; x += 1) {
            ...
            let red = current[offset];
            let green = current[offset + 1];
            let blue = current[offset + 2];
            let weight = 1;
...
function addLocalDetail(data, width, height, amount, detailLimit) {
    if (amount <= 0 || width < 3 || height < 3)
        return;
    const rowBytes = width * 4;
    let previous = data.slice(0, rowBytes);
    ...
    for (let y = 1; y < height - 1; y += 1) {
        ...
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const surround = (luma(current[offset - 4], current[offset - 3], current[offset - 2])
                + luma(current[offset + 4], current[offset + 5], current[offset + 6])
                + luma(previous[offset], previous[offset + 1], previous[offset + 2])
                + luma(next[offset], next[offset + 1], next[offset + 2])) / 4;
...
export function applyManualAdjustments(source, raw, copy = true) {
