// Generated from src/lib/image/scan-filters.ts by sync-scan-filters.mjs.
const clamp = (value) => Math.max(0, Math.min(255, Math.round(value)));
const luma = (r, g, b) => (54 * r + 183 * g + 19 * b) >> 8;
function conservativeWhiteBalance(data) {
    let red = 0;
    let green = 0;
    let blue = 0;
    let samples = 0;
...
function restrainedSharpen(data, width, height, amount) {
    ...
    for (let y = 1; y < height - 1; y += 1) {
        ...
        for (let x = 1; x < width - 1; x += 1) {
            ...
            const neighbors = (luma(current[offset - 4], current[offset - 3], current[offset - 2])
...
function adaptiveThreshold(image, bias) {
...
export function applyScanFilter(source, filter) {
    if (filter === 'original')
        return new ImageData(new Uint8ClampedArray(source.data), source.width, source.height);
    if (filter === 'bw' || filter === 'receipt')
        return adaptiveThreshold(source, filter === 'receipt' ? 13 : 9);
...
export function scanQualityHint(image) {
    ...
    if (average < 58)
        return 'more-light';
    if (bright / Math.max(1, samples) > 0.18)
        return 'glare';
    if (edges / Math.max(1, samples * 2) < 5.5)
        return 'blur';
    return 'hold-steady';
}
