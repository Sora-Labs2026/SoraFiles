...
export class LicenseLedger {
 ...
 CREATE TABLE IF NOT EXISTS paid_replacements(id TEXT PRIMARY KEY,license_ref TEXT NOT NULL REFERENCES licenses(ref),old_device TEXT NOT NULL,new_device TEXT NOT NULL,instance_id TEXT NOT NULL,key_hash TEXT NOT NULL,customer_id TEXT NOT NULL,plan TEXT NOT NULL,amount INTEGER NOT NULL,currency TEXT NOT NULL,product_id TEXT NOT NULL,status TEXT NOT NULL,created INTEGER NOT NULL,checkout_id TEXT UNIQUE,checkout_url TEXT,payment_id TEXT UNIQUE,paid INTEGER,completed INTEGER,activated INTEGER,UNIQUE(license_ref,old_device),UNIQUE(license_ref,new_device));
 CREATE TABLE IF NOT EXISTS replacement_payment_events(id TEXT PRIMARY KEY,payment_id TEXT NOT NULL,received INTEGER NOT NULL,completed INTEGER);
 CREATE TABLE IF NOT EXISTS replacement_email(id TEXT PRIMARY KEY,license_ref TEXT NOT NULL,customer_id TEXT NOT NULL,device_id TEXT NOT NULL,key_hash TEXT NOT NULL,code_hash TEXT NOT NULL,created INTEGER NOT NULL,expires INTEGER NOT NULL,attempts INTEGER NOT NULL,verified INTEGER,token_hash TEXT UNIQUE,token_expires INTEGER,old_device TEXT);
 CREATE INDEX IF NOT EXISTS replacement_email_limits ON replacement_email(license_ref,created);
 `);
 ...
 reserveActivation(keyHash,deviceId,now){if(this.db.prepare("SELECT 1 FROM paid_replacements WHERE key_hash=? AND old_device=? AND status IN ('paid','complete')").get(keyHash,deviceId))throw Error('Old activation replaced');if(this.db.prepare('SELECT 1 FROM support_replacements WHERE key_hash=? AND old_device=?').get(keyHash,deviceId))throw Error('Old activation replaced; contact support');return this.db.prepare("INSERT OR IGNORE INTO activation_attempts(key_hash,device_id,status,created) VALUES(?,?,'pending',?)").run(keyHash,deviceId,now).changes===1;}
 ...
 finishActivation(keyHash,deviceId,status,now=Math.floor(Date.now()/1000),issue){if(!['complete','blocked'].includes(status))throw Error('Invalid activation outcome');return this.transaction(()=>{const row=this.activationAttempt(keyHash,deviceId);let result;if(status==='complete'){if(!row?.license_ref||!this.active(row.license_ref,deviceId))throw Error('Missing activation to bind');if(issue)result=issue(this.currentLicense(row.license_ref,deviceId,row.instance_id,now));this.db.prepare('INSERT OR IGNORE INTO permanent_devices VALUES(?,?)').run(row.license_ref,deviceId);this.db.prepare("UPDATE paid_replacements SET activated=COALESCE(activated,?) WHERE license_ref=? AND new_device=? AND status='complete'").run(now,row.license_ref,deviceId);this.db.prepare("UPDATE support_replacements SET activated=COALESCE(activated,?) WHERE license_ref=? AND new_device=? AND status='complete'").run(now,row.license_ref,deviceId);}this.db.prepare('UPDATE activation_attempts SET status=? WHERE key_hash=? AND device_id=?').run(status,keyHash,deviceId);return result;});}
 currentLicense(ref,deviceId,instanceId,now){
  ...
  if(this.db.prepare("SELECT 1 FROM paid_replacements WHERE license_ref=? AND old_device=? AND status IN ('paid','complete')").get(ref,deviceId))throw Error('Old activation replaced');
 ...
 activate(ref,deviceId,instanceId,now,{existingOnly=false,provisional=false}={}){return this.transaction(()=>{const license=this.db.prepare('SELECT * FROM licenses WHERE ref=?').get(ref);if(!license||license.status!=='active')throw Error('License inactive');const plan=plans[license.plan];if(plan.interval!=='lifetime'&&(!license.period_end||license.period_end<=now))throw Error('Subscription expired');
  ...
  if(this.db.prepare("SELECT 1 FROM paid_replacements WHERE license_ref=? AND new_device=? AND status<>'complete'").get(ref,deviceId))throw Error('Paid replacement is pending');
