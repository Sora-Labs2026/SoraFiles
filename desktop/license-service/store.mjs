...
export class LicenseStore {
 ...
 finishActivation(keyHash,deviceId,status){if(!['complete','blocked'].includes(status))throw Error('Invalid activation outcome');this.transaction(()=>{const row=this.activationAttempt(keyHash,deviceId);if(status==='complete'){if(!row?.license_ref||!this.active(row.license_ref,deviceId))throw Error('Missing activation to bind');this.db.prepare('INSERT OR IGNORE INTO permanent_devices VALUES(?,?)').run(row.license_ref,deviceId);}this.db.prepare('UPDATE activation_attempts SET status=? WHERE key_hash=? AND device_id=?').run(status,keyHash,deviceId);});}
 ...
 activate(ref,deviceId,instanceId,now,{existingOnly=false,provisional=false}={}){return this.transaction(()=>{const license=this.db.prepare('SELECT * FROM licenses WHERE ref=?').get(ref);if(!license||license.status!=='active')throw Error('License inactive');const plan=plans[license.plan];if(plan.interval!=='lifetime'&&(!license.period_end||license.period_end<=now))throw Error('Subscription expired');
  ...
  const {count}=this.db.prepare('SELECT COUNT(*) AS count FROM (SELECT device_id FROM devices WHERE license_ref=? AND active=1 UNION SELECT device_id FROM permanent_devices WHERE license_ref=?)').get(ref,ref);if(count>=plan.maxDevices)throw Error('Activation limit reached; license seats are permanently device-bound');
  this.db.prepare('INSERT INTO devices VALUES(?,?,?,1) ON CONFLICT(license_ref,device_id) DO UPDATE SET instance_id=excluded.instance_id,active=1').run(ref,deviceId,instanceId);if(!provisional)this.db.prepare('INSERT OR IGNORE INTO permanent_devices VALUES(?,?)').run(ref,deviceId);return license;
 });}
