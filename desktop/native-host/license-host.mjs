...
export async function runLicenseAction({action,params={},state,config,saveState,fetchImpl}) {
 if(!['status','trial','activate','refresh','deactivate','devices'].includes(action)||!params||typeof params!=='object'
 ...
 if(action==='status'){try{const result=await client.authorize();return {license:result.plan==='trial'?'trial':'active',...result};}catch{return {license:current.license?.deactivationPending?'deactivation-pending':'needs-verification'};}}
 ...
 return {license:action==='deactivate'?'not-activated':result.plan==='trial'?'trial':'active',...result};
}
