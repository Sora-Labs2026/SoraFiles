...
- Eight headless PDF operations (merge, split, rotate, remove pages, page numbers, watermark, visible signature, images to PDF) are connected to the signed-entitlement reference queue. Earlier independent validation checked 177 pages for merge, split, rotate and remove pages; newer operations have separate scoped output tests. Native UI processing is not connected yet.
...
Run `node --test desktop/tests/*.test.mjs`. All 101 current tests pass using synthetic keys, files and Dodo responses. Seven additional actual Dodo Test Mode lifecycle checks now pass; see `docs/dodo-test-setup.md`. PDF input reads use fixed allocations, chunk-level cancellation and the remaining aggregate job budget; growth/truncation is rejected. A published output remains successful if staging cleanup or subsequent engine housekeeping fails; collision errors before publication still fail safely. Separately, actual Dodo Test Mode checkout summaries for all six plans matched the approved USD totals with 10% GST included. The owner reports manually creating and using discount codes in test checkouts. The agent has not submitted a purchase; production charges and real license-email delivery remain untested. Dodo documents that Test Mode suppresses transactional emails. See `docs/dodo-test-setup.md`.
...
3. Extract and validate the remaining 14 processing workflows; connect the existing independent desktop UI, native dialogs, licensed jobs, output actions and batch UX.
...
The native-host license client adapter now verifies device-bound trial and paid activation responses over HTTP, keeps sensitive license fields out of UI responses, and authorizes signed grants offline. Its tests use a local service, synthetic keys, simulated Dodo and in-memory protected-storage callbacks. The native licensing bridge and OS-protected storage are now connected. A Rust private-pipe integration test verifies trial activation and offline authorization; native paid activation and production deployment remain unfinished. The latest owner steering removes the third-party trial sign-in requirement.
