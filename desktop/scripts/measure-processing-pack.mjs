...
async function visit(directory){for(const entry of await readdir(directory,{withFileTypes:true})){
 const path=join(directory,entry.name);if(entry.isSymbolicLink())throw Error('Unexpected link in generated pack: '+relative(root,path).split(sep).join('/'));
...
const matchedBaseline=baseline.platform===process.platform&&baseline.arch===process.arch;
const report={recordedAt:new Date().toISOString(),platform:process.platform,arch:process.arch,scope:'Generated resource pack; not installer size or measured installed footprint',bytes,files,groups:[...groups].sort((a,b)=>b[1]-a[1]),baselineStatus:matchedBaseline?'same-platform-comparison':'pending-first-platform-baseline',...(matchedBaseline?{baselineBytes:baseline.bytes,savedBytes:baseline.bytes-bytes,reductionPercent:(baseline.bytes-bytes)/baseline.bytes*100}:{baselineBytes:null,baselineReason:'No reviewed size baseline exists for this platform and architecture; the Windows baseline is not applicable.'})};
