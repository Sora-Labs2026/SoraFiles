...
const tools=liveTools.map(t=>({id:t.id,name:t.name,route:'/'+t.slug,engine:t.engine||'Canvas / shared image adjustments',inputFormats:t.inputFormats,outputFormats:t.outputFormats,
 ...
 desktopPath:'Engine extraction and native-host prototype pending; not claimed shipped.',releaseCleared:false}));
...
const result={recordedAt:new Date().toISOString(),status:'RELEASE BLOCKED: source/compliance packs and native-host prototypes incomplete',registryHash:createHash('sha256').update(await readFile(new URL('src/data/liveTools.ts',root))).digest('hex'),tools,packages,productionLockEntries:dependencies,assets,localPublicBytes:assets.reduce((n,a)=>n+a.bytes,0)};
