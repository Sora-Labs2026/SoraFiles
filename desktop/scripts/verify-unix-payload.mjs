...
const run=async(program,args,cwd)=>new Promise((done,fail)=>{
 ...
 child.once('close',(code,signal)=>{clearTimeout(timer);if(code!==0||timedOut)fail(Error('Package verification command failed: '+(timedOut?'timeout':signal||code)+' '+stderr));else done(stdout.trim());});
});
