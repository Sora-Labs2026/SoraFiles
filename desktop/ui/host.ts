...
const publicResult=(value:any)=>value&&typeof value.license==='string'?{trialPending:false,...value}:value;
...
export function host(method:string,params:Record<string,unknown>={},files?:File[]):Promise<any>{
 if(native){if(files)return Promise.reject(Error('Use Choose files if native file drop is unavailable.'));return native.core.invoke('host_request',{method,params}).then(publicResult).catch(error=>{throw Error(typeof error==='string'?error:'This action could not finish.');});}
...
export function onLicenseUpdated(callback:(status:any)=>void){if(native)void native.event.listen('license-updated',event=>callback(event.payload));}
